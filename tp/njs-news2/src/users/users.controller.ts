...
export class UsersController {
    ...
    async create(@Body() user: UserL0Dto): Promise<UserL1Dto> {
     ...
     async remove(@Param('id') id:string): Promise<any> {
       let deletedOk = await this.userService.remove(id);
    ...
    async update(@Body() userToUpdate: UserL1Dto, @Param('id') id:string): Promise<UserL1Dto> {
        userToUpdate.id=id; //must be coherent
        const updatedOk =   await this.userService.update(id, userToUpdate);
        console.log(`updatedOk=${updatedOk} in UserController.update()`)
        let updatedUser = await this.userService.findOne(id);
        return updatedUser; //with default 200/ok
        //ErrorExceptionFilter may return NOT_FOUND if necessary
    }
